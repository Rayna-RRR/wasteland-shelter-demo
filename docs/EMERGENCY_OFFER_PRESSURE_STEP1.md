# Emergency Offer Pressure Step 1

## 旧触发问题

旧版 emergency offer 已经有资源、队伍、近期行动效率三类判断，但触发原因比较笼统：
...
- 行动效率：最近 3 条非休整 duty logs 中，2 条总资源变化 `<= 2` 即触发。

## 新触发分类

Day 2 Step 1 改为先判断实际压力来源，再决定 offer 是否出现：
...
- 行动压力：至少完成 6 次非休整值勤，并且最近 4 条非休整 duty logs 中有 3 条总资源变化 `<= 2`。
...
- 用户关闭 offer 后，后端仍会写入 `closed` 日志。
- 关闭后仍保留按天抑制和按行动次数抑制，避免 offer 立刻重复出现。
- 如果出现严重压力，`severe_pressure_override` 可以让 offer 不再被普通关闭抑制挡住。
...
4. 检查购买接口在 offer 激活时是否仍按原逻辑发放奖励：
