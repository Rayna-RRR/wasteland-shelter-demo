...
当前事件系统是演示级实现，重点是展示“资源压力 + 人员状态 + 玩家选择”的闭环。
...
当前采用“每日事件池轮换 + 条件过滤”的轻量实现。结果可预测，便于录屏、测试和复盘。
...
设计作用：让高压决策有长期后果，避免事件全部停留在短期资源加减。
...
- 事件池很小，目前只有 7 个事件。
- 当前选择逻辑按天数轮换有效事件，尚未接入权重随机。
- 数值平衡是演示级别，事件收益和惩罚还需要更多实测样本。
...
- 后续如果接入大模型，只用于生成可选事件文案；核心数值结算继续由本地规则控制。
